...
\abstract{The use of accelerators, such as GPUs, to reduce the execution time of GPGPU applications has become more popular during last years. 
These devices increment the computational power of a node thanks to its parallel architecture.
This trend has led several cloud vendors such as Amazon Web Services or some middlewares such as OpenStack to add to their current facilities, or to allow, 
instances of virtual machines equipped with NVIDIA GPUs. 
Accomplishing these needs, the guest hosts must be equipped with GPUs which could be barely utilized if a non GPU-enabled Virtual Machine is running in the host.
The solution presented in this work is based on GPU virtualization and shareability in order to reach an equilibrium between supply and demand of accelerators. 
Hence, we propose to decouple from the nodes, real GPUs by using the virtualization technology called rCUDA. 
With this software configuration, the GPUs can be accessed from any VM getting rid 
...
The growth in the use of coprocessor and accelerators has been continuous
in the past several years. In the most recent TOP500
ranking~\cite{nov14top500},  dated June 2015, 90 supercomputers
were equipped with accelerators of some sort.  These devices offer
performance improvements over traditional processors for
computational kernels which match their architecture; moreover, 
they are more energy efficient achieving an important MFlop/Watt ratio.

The generalized adoption of coprocessors to accelerate general-purpose 
codes started with the release of the CUDA programming model for
...
accelerators.  
%Prior to that, researchers matched 
%GPU architectures to computational kernels, but these had to
%use graphics-oriented APIs such as OpenGL~\cite{opengl} or
%Cg~\cite{cg} to do so. 
Following CUDA, OpenCL~\cite{opencl} appeared as an attempt
to offer a cross-vendor solution. The third-generation programming
model for accelerators, based on compiler directives, started with
OpenACC~\cite{openaccweb}. This standard defines a collection of 
directives to facilitate platform-independent accelerator
usage. This generation is also completed with other task-oriented, directive-based programming model such as OmpSs~\cite{ompssweb} 
which can target directly CPUs or accelerators and recently, OpenMP~\cite{openmp4} incorporated some
accelerator-oriented directives in its version 4.
%, although 
%no available compiler supports them to date.

The current approach for creating clusters with GPUs has been to
populate every compute node with one or more of these devices. 
However, these configurations present a low
utilization rate of the computational resources available in the
hardware accelerators, not only because of mismatches between the application's type of parallelism
and the coprocessor architecture, but also because of applications can be hardly parallelized more than 
80\% of the total code. 
In order to address this problem, remote virtualization was proposed.
Among the virtualization frameworks, the most
prominent is \rcuda~\cite{tonithesis,toniparco}, which
enables cluster configurations with fewer GPUs than nodes.  With this
approach, GPU-equipped nodes act as GPGPU (general-purpose GPU) servers for the rest of the
compute nodes of the cluster, enabling a CUDA-sharing solution that potentially achieves 
 a higher overall GPU load in the system. Compared with 
other CUDA and OpenCL virtualization frameworks (e.g.
DS-CUDA~\cite{dscuda}, vCUDA~\cite{vcuda}, VOCL~\cite{vocl}, and SnuCL~\cite{snucl}),
 \rcuda\ is a mature production-ready framework
 that offers support for the latest CUDA revisions and provides wide
coverage of current GPGPU APIs.


Many cloud vendors have started offering VM with GPUs in order to provide 

Accelerators such as GPUs have been adopted by major cloud vendors,
 
as GPUs provide orders-of-magnitude speedup for computation-intensive data-parallel applications. 

In cloud environments, sharing GPU resources among multiple virtual machines (VMs) is not straightforward which is translated into a low utilization rate.
...
accelerate general-purpose workloads in many applica-
tion domains such as supercomputing systems. Unfor-
tunately, enterprise and cloud computing domains have

inefficient access to GPU virtualization technology be-
cause the required resource isolation of multiple vir-
tual machines (VMs) accessing the same set of GPU re-
sources cannot be efficiently guaranteed.

Efficiently utilizing GPU resources in the cloud envi-
ronment is not so straightforward. Current commercial

cloud providers (e.g., Amazon Elastic Compute Cloud—

EC2) dedicate an individual physical instance of GPUs

to a VM, which causes resource under-utilization. A fea-
sible solution to make GPU a truly shared and schedula-
ble resource in the cloud is through GPU virtualization,

which allows multiple VMs to access the same set of

GPUs simultaneously. Recent research [8, 9, 14, 20, 26,

27] implements several GPU virtualization technologies

for graphics rendering applications as well as GPGPU
