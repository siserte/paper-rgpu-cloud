...
instances. 
To fullfil these needs, the guest hosts must be equipped with GPUs which, unfortunately, will be barely utilized if a non GPU-enabled VM is running in the host.
...
As shown in the Table~\ref{table:bwt}, the bandwidth achieved in this test is higher than the network bandwith, revealing that the accelerator is an actual GPU. 
% \begin{table}[htb]
% \renewcommand{\arraystretch}{1.3}
