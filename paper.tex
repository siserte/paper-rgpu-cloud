...
AWS forces the user to choose between a poor instance with a GPU and a powerful instance with 4 GPUs.
...
If needed, we could work on an instance of 2 GPUs for only \$ 1.3 by launching 2 ``g2.2xlarge'' and using remote GPUs. Avoiding to pay the double for features that we are not willing to use in ``g2.8xlarge''.
