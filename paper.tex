...
the computational power of a computer thanks to its parallel architecture. %The popularity of the GPGPU computation 
...
Hence, we propose to decouple, from the nodes, real GPUs by using the virtualization technology. 
So, these GPUs can be accessed from any VM getting rid of the necessity of having physical 
GPUs in the guest host. 
...
%goes further and it takes into account that by making logical 
%partitions of the GPU memory we could address each partition as if it was an 
%independent GPU device.
}
...
that the guest host must be equipped with at least a GPU, and
...
\section{Working with Remote GPUs in AWS}
\label{sec:rgpuaws}

\begin{itemize}
\item Esc\begin{figure}[!t]
  \centering
  \includegraphics[width=\linewidth]{images/AWS.pdf}
  \caption{An AWS instance with 13 remote GPUs}
  \label{fig:aws1}
\end{figure}enarios creados
...
In this section is explained all the work related to OpenStack that has been done, from the development of an external module to handle remote GPUs to the physical setup and its testing.
...
\begin{figure}[!t]
  \centering
  \includegraphics[width=\linewidth]{images/UI-launch.jpg}
...
\begin{figure}[!t]
  \centering
  \includegraphics[width=\linewidth]{images/UI-rgpus.jpg}
